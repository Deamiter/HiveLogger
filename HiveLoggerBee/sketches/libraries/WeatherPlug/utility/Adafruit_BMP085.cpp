...
uint8_t Adafruit_BMP085::read8(uint8_t a) {
  ...
  I2C_BEGIN_TRANSMIT(BMP085_I2CADDR); // start transmission to device 
#if (ARDUINO >= 100)
  I2C_WRITE((byte)a); // sends register address to read from
#else
  I2C_WRITE((byte)a); // sends register address to read from
#endif
  I2C_END_TRANSMIT(); // end transmission
  delay(5);

  I2C_BEGIN_TRANSMIT(BMP085_I2CADDR); // start transmission to device 
  I2C_REQUEST(BMP085_I2CADDR, 1);// send data n-bytes read
  delay(1);
#if (ARDUINO >= 100)
...
uint16_t Adafruit_BMP085::read16(uint8_t a) {
  ...
  I2C_BEGIN_TRANSMIT((byte)BMP085_I2CADDR); // start transmission to device 
#if (ARDUINO >= 100)
  I2C_WRITE((byte)a); // sends register address to read from
#else
  I2C_WRITE((byte)a); // sends register address to read from
  ...
  I2C_BEGIN_TRANSMIT((byte)BMP085_I2CADDR); // start transmission to device 
  I2C_REQUEST(BMP085_I2CADDR, 2);// send data n-bytes read
delay(1);
#if (ARDUINO >= 100)
  ret = I2C_READ(); // receive DATA
  ret <<= 8;
